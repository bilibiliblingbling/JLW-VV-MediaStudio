...
void FancyToolButton::paintEvent(QPaintEvent *event)
{
    ...
    if (!HostOsInfo::isMacHost() // Mac UIs usually don't hover
        ...
        painter.save();
        const QColor hoverColor = mediastudioTheme()->color(Theme::FancyToolButtonHoverColor);
        ...
        painter.save();
        const QColor selectedColor = mediastudioTheme()->color(Theme::FancyToolButtonSelectedColor);
        if (mediastudioTheme()->widgetStyle() == Theme::StyleDefault) {
            QLinearGradient grad(rect().topLeft(), rect().topRight());
            grad.setColorAt(0, Qt::transparent);
...
void FancyActionBar::paintEvent(QPaintEvent *event)
{
    ...
    if (mediastudioTheme()->widgetStyle () == Theme::StyleFlat) {
        ...
        // left tab bar
        painter.fillRect(event->rect(), mediastudioTheme()->color(Theme::FancyTabBarBackgroundColor));
    }
