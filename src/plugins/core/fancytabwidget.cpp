...
void FancyTabBar::paintEvent(QPaintEvent *event)
{
    ...
    if (mediastudioTheme()->widgetStyle() == Theme::StyleFlat) {
        ...
        // (Welcome, ... Help)
        p.fillRect(event->rect(), mediastudioTheme()->color(Theme::FancyTabBarBackgroundColor));
    }
...
void FancyTabBar::paintTab(QPainter *painter, int tabIndex) const
{
    ...
    if (selected) {
        if (mediastudioTheme()->widgetStyle() == Theme::StyleFlat) {
          ...
          painter->fillRect(rect.adjusted(0, 0, 0, -1),
                            mediastudioTheme()->color(Theme::BackgroundColorSelected));
        } else {
            //background
            painter->save();
    ...
    if (!selected && enabled) {
        ...
        if (mediastudioTheme()->widgetStyle() == Theme::StyleFlat) {
            QColor c = mediastudioTheme()->color(Theme::BackgroundColorHover);
    ...
    if (enabled) {
        painter->setPen(selected
          ? mediastudioTheme()->color(Theme::FancyTabWidgetEnabledSelectedTextColor)
          : mediastudioTheme()->color(Theme::FancyTabWidgetEnabledUnselectedTextColor));
    } else {
        painter->setPen(selected
          ? mediastudioTheme()->color(Theme::FancyTabWidgetDisabledSelectedTextColor)
          : mediastudioTheme()->color(Theme::FancyTabWidgetDisabledUnselectedTextColor));
    }
