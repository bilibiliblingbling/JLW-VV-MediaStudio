...
// TODO: fix required, need to remove unused items
...
{
    setObjectName(QLatin1String("Core.ICore"));
...
{
    return qApp->property("mediastudio_locale").toString();
}

QString ICore::resourcePath()
{
    // TODO: user resource path, set in .pri
    const QString sharePath = QLatin1String("/../share/") + QLatin1String(MEDIASTUDIO_PREFIX);
...
QString ICore::userResourcePath()
{
    ...
    const QString urp = configDir + QLatin1Char('/') + QLatin1String(MEDIASTUDIO_PREFIX);
...
{
    // TODO: set in .pri
    const QString docPath = QLatin1String("/../share/doc/") + QLatin1String(MEDIASTUDIO_PREFIX);
...
{
    // TODO: set in .pri
    QString path = QCoreApplication::applicationDirPath() + QLatin1String("/../libexec") + QLatin1String(MEDIASTUDIO_PREFIX);
...
Pane *ICore::browsePane()
{
    return m_mainwindow->browsePane();
}

Pane *ICore::monitorPane()
{
    return m_mainwindow->monitorPane();
}

Pane *ICore::previewPane()
{
    return m_mainwindow->previewPane();
}

Pane *ICore::timelinePane()
{
    return m_mainwindow->timelinePane();
}
