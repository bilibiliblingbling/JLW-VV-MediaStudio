...
bool PluginSpecPrivate::read(const QString &fileName)
{
    ...
    if (loader.fileName().isEmpty()) {
        qCDebug(plugin_log) << "Cannot open file";
        return false;
    }
...
{
    if (!out) {
        qCWarning(plugin_log) << Q_FUNC_INFO << "missing output parameter";
        return false;
    }
    if (value.isString()) {
        *out = value.toString();
...
{
    qCDebug(plugin_log) << "MetaData:" << QJsonDocument(metaData).toJson();
    QJsonValue value;
    value = metaData.value(QLatin1String("IID"));
    if (!value.isString()) {
        qCDebug(plugin_log) << "Not a plugin (no string IID found)";
        return false;
    }
    if (value.toString() != PluginManager::pluginIID()) {
        qCDebug(plugin_log) << "Plugin ignored (IID does not match)";
        return false;
    }
    ...
    qCDebug(plugin_log) << "required =" << required;
    ...
    qCDebug(plugin_log) << "experimental =" << experimental;
    ...
    qCDebug(plugin_log) << "enabledByDefault =" << enabledByDefault;
    ...
    if (!value.isUndefined()) {
        ...
        foreach (const QJsonValue &v, array) {
            ...
            qCDebug(plugin_log) << "Argument:" << arg.name << "Parameter:" << arg.parameter
